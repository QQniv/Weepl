...
function fromKey(iso) { var p = (iso || '1970-01-01').split('-'); return new Date(Number(p[0]), Number(p[1]) - 1, Number(p[2])); }
...
function safeFocus(el) { try { el && el.focus && el.focus({ preventScroll: true }); } catch (e) {} }
...
function mountTemplate(where, tplId) {
  var root = typeof where === 'string' ? $(where) : where;
  var tpl = $(tplId);
  if (!root || !tpl) return;
  root.innerHTML = '';
...
function router() {
  try {
    ...
    if (!routes[h]) h = '#/';
    // монтируем шаблон внутри рендера
    routes[h](view);
    setActiveTab();
    safeFocus(view);
  } catch (err) {
...
function showCrash(err) {
  var app = $('#app');
  if (!app) return;
  app.innerHTML = '<div style="padding:16px;font:14px/1.45 ui-monospace, SFMono-Regular, Menlo, monospace;color:#b00020;background:#fff3f3;border:1px solid #ffd8d8;border-radius:10px;margin:16px;">'+
    '<b>Ошибка рендера</b><br>'+escapeHTML(String(err && err.message || err))+'</div>';
  console.error(err);
}

/* ========== App State ========== */
var UI = { home: { month: new Date(), selected: dtKey(new Date()) }, cal: { current: new Date(), selected: dtKey(new Date()) } };

/* ========== Boot ========== */
(function init() {
  try {
    var app = $('#app'), layoutTpl = $('#layout');
    if (!app || !layoutTpl) return;
    app.appendChild(document.importNode(layoutTpl.content, true));

    var st = Store.get().settings;
    applyTheme(st.theme || 'light'); applyAccent(st.accent || 'blue');

    var modal = $('#taskModal'), form = $('#taskForm');
    var fab = $('#fabAdd'); if (fab) fab.addEventListener('click', function () { openTaskModal({ date: UI.home.selected }); });
    $$('[data-close]').forEach(function (b) { b.addEventListener('click', function () { modal && modal.close(); }); });
    if (modal) modal.addEventListener('cancel', function (e) { e.preventDefault(); modal.close(); });
    if (form) form.addEventListener('submit', onSubmitTask);

    document.addEventListener('keydown', function (e) {
      if (e.key && e.key.toLowerCase() === 'n' && modal && !modal.open) openTaskModal({ date: UI.home.selected });
      if (e.key === 'Escape' && modal && modal.open) modal.close();
    });

    document.addEventListener('store:change', function () {
      var h = location.hash || '#/';
      if (h === '#/') renderHome($('#view'));
      if (h === '#/calendar') renderCalendar($('#view'));
    });
...
function renderHome(root) {
  mountTemplate(root, '#home-view');

  var monthTitleEl = $('#homeMonth');
  if (monthTitleEl) monthTitleEl.textContent = monthTitle(UI.home.month);

  var prev = $('[data-home-month="prev"]'), next = $('[data-home-month="next"]');
  if (prev) prev.onclick = function () { UI.home.month = addMonths(UI.home.month, -1); $('#homeMonth').textContent = monthTitle(UI.home.month); paintDateStrip(); };
  if (next) next.onclick = function () { UI.home.month = addMonths(UI.home.month, 1); $('#homeMonth').textContent = monthTitle(UI.home.month); paintDateStrip(); };
...
function paintDateStrip() {
  var strip = $('#dateStrip'); if (!strip) return;
  strip.innerHTML = '';
  ...
  for (var i = 0; i < 14; i++) {
    ...
    var key = dtKey(d);
    var wk = d.toLocaleString('ru-RU', { weekday: 'short' }).toUpperCase();
    var active = key === UI.home.selected ? ' active' : '';
    var html = '<button class="date-pill' + active + '" data-key="' + key + '">'+
               '<div class="wk">' + wk + '</div>'+
               '<div class="daynum">' + ('0' + d.getDate()).slice(-2) + '</div>'+
               '</button>';
  ...
  $$('.date-pill', strip).forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      var t = $('#todayTitle'); if (t) t.textContent = humanDate(UI.home.selected);
      paintTodayList();
    });
  });
  var activeEl = $('.date-pill.active', strip); if (activeEl && activeEl.scrollIntoView) activeEl.scrollIntoView({ inline: 'center', block: 'nearest', behavior: 'smooth' });
  var t2 = $('#todayTitle'); if (t2) t2.textContent = humanDate(UI.home.selected);
}

function paintTodayList() {
  var listWrap = $('#todayList'); var list = $('#taskList');
  if (!listWrap || !list) return;
  var tasks = Store.get().tasks.filter(function (t) { return (t.date || '') === UI.home.selected; }).sort(sortByWhen);
  if (!tasks.length) list.innerHTML = '<p class="muted">На этот день пока пусто. Нажми <kbd>N</kbd> или «+», чтобы добавить задачу.</p>';
  else list.innerHTML = tasks.map(taskRow).join('');
  $$('.task', listWrap).forEach(function (card) {
    var tg = card.querySelector('[data-toggle]'), ed = card.querySelector('[data-edit]'), dl = card.querySelector('[data-del]');
    if (tg) tg.addEventListener('click', function () { toggleDone(card.dataset.id); });
    if (ed) ed.addEventListener('click', function () { startEdit(card.dataset.id); });
    if (dl) dl.addEventListener('click', function () { delTask(card.dataset.id); });
    bindSwipe(card);
  });
}
...
var Calendar = {
  ...
  paint: function (now) {
    mountTemplate($('#view'), '#calendar-view');
    var grid = $('#calendarGrid'), title = $('#calTitle'), list = $('#calendarTasks');
    ...
    for (var d = 1; d <= daysInMonth; d++) {
      ...
      var dots = '';
      dayTasks.slice(0, 8).forEach(function (t) { var c = prioColor(t.priority); dots += '<span class="dot" style="background:' + c + ';box-shadow:0 0 10px ' + c + '33"></span>'; });
    ...
    if (prev) prev.onclick = function () { Calendar.move(-1); };
    if (next) next.onclick = function () { Calendar.move(1); };

    $$('.daycell').forEach(function (c) {
      c.addEventListener('click', function () {
        var key = c.getAttribute('data-date'); Calendar.state.selected = key;
        renderCalendarTasks(list, key);
        UI.home.selected = key; var t = $('#todayTitle'); if (t) t.textContent = humanDate(key);
      ...
      c.addEventListener('drop', function (ev) {
        ...
        t.date = c.getAttribute('data-date'); Store.set(s); renderCalendarTasks(list, t.date);
      });
    });
...
function renderSettings(root) {
  mountTemplate(root, '#settings-view');
  var s = Store.get().settings;
  var th = $('#setTheme'), ac = $('#setAccent'), ex = $('#btnExport'), fi = $('#fileImport');
  if (th) { th.value = s.theme; th.onchange = function (e) { s.theme = e.target.value; applyTheme(s.theme); Store.set(Store.get()); }; }
  if (ac) { ac.value = s.accent; ac.onchange = function (e) { s.accent = e.target.value; applyAccent(s.accent); Store.set(Store.get()); }; }
  if (ex) ex.onclick = exportJSON;
  if (fi) fi.onchange = importJSON;
}
...
function onSubmitTask(e) {
  ...
  Store.set(s);
  var modal = $('#taskModal'); if (modal) modal.close(); e.target.reset();
  paintTodayList();
}
...
function openTaskModal(prefill) {
  var modal = $('#taskModal'); if (!modal) return;
  var hid = $('#fId'); if (!hid) { hid = document.createElement('input'); hid.type = 'hidden'; hid.id = 'fId'; hid.name = 'id'; $('#taskForm').appendChild(hid); }
  ...
  modal.showModal(); safeFocus($('#fTitle'));
}
...
function bindSwipe(card) {
  ...
  card.addEventListener('touchend', function () {
    ...
    if (dx > 80) { toggleDone(id); } else if (dx < -80) { var s = Store.get(); var t = s.tasks.find(function (x) { return x.id === id; }); if (t && t.date) { t.date = addDaysIso(t.date, 1); Store.set(s); paintTodayList(); } }
